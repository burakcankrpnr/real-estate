...
export const metadata: Metadata = {
  title: "Satılık Motel İlanları | Tüzgen Group",
  description: "Satılık motel ilanlarını inceleyebilir, turizm sektöründeki yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=satilik&type=motel&limit=24`,
...
export default async function SatilikMotelPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Motel İlanları"
        description="Satılık motel ilanlarını inceleyin ve turizm sektöründeki yatırım fırsatlarını değerlendirin."
      />
