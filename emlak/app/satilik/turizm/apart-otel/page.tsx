...
export const metadata: Metadata = {
  title: "Satılık Apart Otel İlanları | Tüzgen Group",
  description: "Satılık apart otel ilanlarını inceleyebilir, turizm sektöründeki yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=satilik&type=apart-otel&limit=24`,
...
export default async function SatilikApartOtelPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Apart Otel İlanları"
        description="Satılık apart otel ilanlarını inceleyin ve turizm sektöründeki yatırım fırsatlarını değerlendirin."
      />
