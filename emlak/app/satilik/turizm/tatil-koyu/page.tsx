...
export const metadata: Metadata = {
  title: "Satılık Tatil Köyü İlanları | Tüzgen Group",
  description: "Satılık tatil köyü ilanlarını inceleyebilir, turizm sektöründeki yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=satilik&type=tatil-koyu&limit=24`,
...
export default async function SatilikTatilKoyuPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Tatil Köyü İlanları"
        description="Satılık tatil köyü ilanlarını inceleyin ve turizm sektöründeki yatırım fırsatlarını değerlendirin."
      />
