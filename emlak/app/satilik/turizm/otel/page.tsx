...
export const metadata: Metadata = {
  title: "Satılık Otel İlanları | Tüzgen Group",
  description: "Satılık otel ilanlarını inceleyebilir, turizm sektöründeki yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=satilik&type=otel&limit=24`,
...
export default async function SatilikOtelPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Otel İlanları"
        description="Satılık otel ilanlarını inceleyin ve turizm sektöründeki yatırım fırsatlarını değerlendirin."
      />
