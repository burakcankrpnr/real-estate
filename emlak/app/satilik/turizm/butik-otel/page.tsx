...
export const metadata: Metadata = {
  title: "Satılık Butik Otel İlanları | Tüzgen Group",
  description: "Satılık butik otel ilanlarını inceleyebilir, turizm sektöründeki yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=satilik&type=butik-otel&limit=24`,
...
export default async function SatilikButikOtelPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Butik Otel İlanları"
        description="Satılık butik otel ilanlarını inceleyin ve turizm sektöründeki yatırım fırsatlarını değerlendirin."
      />
