...
export const metadata: Metadata = {
  title: "Satılık Pansiyon İlanları | Tüzgen Group",
  description: "Satılık pansiyon ilanlarını inceleyebilir, turizm sektöründeki yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=satilik&type=pansiyon&limit=24`,
...
export default async function SatilikPansiyonPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Pansiyon İlanları"
        description="Satılık pansiyon ilanlarını inceleyin ve turizm sektöründeki yatırım fırsatlarını değerlendirin."
      />
