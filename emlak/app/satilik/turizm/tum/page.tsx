...
export const metadata: Metadata = {
  title: "Satılık Turizm Tesisleri | Tüzgen Group",
  description: "Satılık otel, pansiyon, tatil köyü ve diğer turizm tesisi ilanlarını keşfedin.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=satilik&limit=24`,
...
export default async function SatilikTurizmPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Turizm Tesisleri"
        description="Satılık otel, pansiyon, tatil köyü ve diğer turizm tesisi ilanlarını keşfedin."
      />
