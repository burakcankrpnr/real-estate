...
export const metadata: Metadata = {
  title: "Satılık Kamp Yeri İlanları | Tüzgen Group",
  description: "Satılık kamp yeri ilanlarını inceleyebilir, turizm sektöründeki yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=satilik&type=kamp-yeri&limit=24`,
...
export default async function SatilikKampYeriPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Kamp Yeri İlanları"
        description="Satılık kamp yeri ilanlarını inceleyin ve turizm sektöründeki yatırım fırsatlarını değerlendirin."
      />
