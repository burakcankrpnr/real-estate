...
export const metadata: Metadata = {
  title: "Satılık Depo Arazisi İlanları | Tüzgen Group",
  description: "Satılık depo arazisi ilanlarını inceleyebilir, yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=satilik&type=depo&limit=24`,
...
export default async function SatilikDepoAraziPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Depo Arazisi İlanları"
        description="Satılık depo arazisi ilanlarını inceleyin ve yatırım fırsatlarını değerlendirin."
      />
