...
export const metadata: Metadata = {
  title: "Satılık Zeytinlik İlanları | Tüzgen Group",
  description: "Satılık zeytinlik arazisi ilanlarını inceleyebilir, yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=satilik&type=zeytinlik&limit=24`,
...
export default async function SatilikZeytinlikPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Zeytinlik İlanları"
        description="Satılık zeytinlik arazisi ilanlarını inceleyin ve yatırım fırsatlarını değerlendirin."
      />
