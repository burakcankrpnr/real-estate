...
export const metadata: Metadata = {
  title: "Satılık Tarla İlanları | Tüzgen Group",
  description: "Satılık tarla ilanlarını inceleyin, bütçenize uygun yatırımlık arazi bulun.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?type=arsa&subtype=tarla&status=satilik&limit=12`,
...
export default async function SatilikTarlaPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Tarla İlanları"
        description="Satılık tarla ilanlarını inceleyin, yatırım için uygun arazileri keşfedin."
      />
