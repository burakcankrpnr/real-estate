...
export const metadata: Metadata = {
  title: "Satılık Çiftlik Arazisi İlanları | Tüzgen Group",
  description: "Satılık çiftlik arazisi ilanlarını inceleyebilir, yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=satilik&type=ciftlik&limit=24`,
...
export default async function SatilikCiftlikAraziPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Çiftlik Arazisi İlanları"
        description="Satılık çiftlik arazisi ilanlarını inceleyin ve yatırım fırsatlarını değerlendirin."
      />
          ...
          <div className="flex flex-wrap justify-between">
            <div className="mb-8 w-full">
              <h2 className="text-2xl font-bold text-black dark:text-white">
                Satılık Çiftlik Arazisi İlanları
              </h2>
            </div>
          </div>
