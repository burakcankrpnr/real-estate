...
export const metadata: Metadata = {
  title: "Satılık Bahçe Arazisi İlanları | Tüzgen Group",
  description: "Satılık bahçe arazisi ilanlarını inceleyebilir, yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=satilik&type=bahce&limit=24`,
...
export default async function SatilikBahcePage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Bahçe Arazisi İlanları"
        description="Satılık bahçe arazisi ilanlarını inceleyin ve yatırım fırsatlarını değerlendirin."
      />
          ...
          <div className="flex flex-wrap justify-between">
            <div className="mb-8 w-full">
              <h2 className="text-2xl font-bold text-black dark:text-white">
                Satılık Bahçe Arazisi İlanları
              </h2>
            </div>
          </div>
