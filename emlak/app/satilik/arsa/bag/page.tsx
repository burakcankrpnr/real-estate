...
export const metadata: Metadata = {
  title: "Satılık Bağ Arazisi İlanları | Tüzgen Group",
  description: "Satılık bağ arazisi ilanlarını inceleyebilir, yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=satilik&type=bag&limit=24`,
...
export default async function SatilikBagPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Bağ Arazisi İlanları"
        description="Satılık bağ arazisi ilanlarını inceleyin ve yatırım fırsatlarını değerlendirin."
      />
