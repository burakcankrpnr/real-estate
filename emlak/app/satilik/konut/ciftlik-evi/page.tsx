...
export const metadata: Metadata = {
  title: "Satılık Çiftlik Evi İlanları | Tüzgen Group",
  description: "Satılık çiftlik evi ilanlarını inceleyebilir, kırsal yaşam fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=satilik&type=ciftlik-evi&limit=24`,
...
export default async function SatilikCiftlikEviPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Çiftlik Evi İlanları"
        description="Satılık çiftlik evi ilanlarını inceleyin ve kırsal yaşam fırsatlarını değerlendirin."
      />
