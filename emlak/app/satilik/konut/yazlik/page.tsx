...
export const metadata: Metadata = {
  title: "Satılık Yazlık İlanları | Tüzgen Group",
  description: "Satılık yazlık ilanlarını inceleyebilir, tatil bölgelerindeki yazlık ev fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=satilik&type=yazlik&limit=24`,
...
export default async function SatilikYazlikPage() {
  ...
  return (
    <>
      <Breadcrumb
        pageName="Satılık Yazlık İlanları"
        description="Tatil bölgelerindeki yazlık ev fırsatlarını inceleyin ve hayalinizdeki yazlık evi bulun."
      />
