...
export const metadata: Metadata = {
  title: "Satılık Prefabrik Ev İlanları | Tüzgen Group",
  description: "Satılık prefabrik ev ilanlarını inceleyebilir, ekonomik ve hızlı teslim seçeneklerini değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=satilik&type=prefabrik-ev&limit=24`,
...
export default async function SatilikPrefabrikEvPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Prefabrik Ev İlanları"
        description="Satılık prefabrik ev ilanlarını inceleyin ve ekonomik konut seçeneklerini değerlendirin."
      />
