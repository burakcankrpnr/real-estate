...
export const metadata: Metadata = {
  title: "Satılık Müstakil Ev İlanları | Tüzgen Group",
  description: "Satılık müstakil ev ilanlarını inceleyin, bütçenize uygun müstakil evi bulun.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?type=konut&subtype=mustakil-ev&status=satilik&limit=12`,
...
export default async function SatilikMustakilEvPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Müstakil Ev İlanları"
        description="Satılık müstakil ev ilanlarını inceleyin, bütçenize ve tarzınıza uygun müstakil evi bulun."
      />
