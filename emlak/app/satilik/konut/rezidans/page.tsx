...
export const metadata: Metadata = {
  title: "Satılık Rezidans İlanları | Tüzgen Group",
  description: "Satılık rezidans ilanlarını inceleyin, bütçenize uygun rezidans dairesini bulun.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?type=konut&subtype=rezidans&status=satilik&limit=12`,
...
export default async function SatilikRezidansPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Rezidans İlanları"
        description="Satılık rezidans ilanlarını inceleyin, bütçenize ve tarzınıza uygun rezidans dairesini bulun."
      />
