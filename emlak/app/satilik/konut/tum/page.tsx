...
export const metadata: Metadata = {
  title: "Satılık Konut İlanları | Tüzgen Group",
  description: "Satılık daire, villa, müstakil ev ve diğer konut ilanlarını inceleyebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=satilik&limit=24`,
...
export default async function SatilikKonutPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Konut İlanları"
        description="Tüm satılık konut ilanlarını inceleyin, size uygun evi bulun."
      />
