...
export const metadata: Metadata = {
  title: "Satılık Akaryakıt İstasyonu İlanları | Tüzgen Group",
  description: "Satılık akaryakıt istasyonu ilanlarını inceleyebilir, ticari yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=satilik&type=akaryakit-istasyonu&limit=24`,
...
export default async function SatilikAkaryakitIstasyonuPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Akaryakıt İstasyonu İlanları"
        description="Satılık akaryakıt istasyonu ilanlarını inceleyin ve ticari yatırım fırsatlarını değerlendirin."
      />
          ...
          <div className="flex flex-wrap justify-between">
            <div className="mb-8 w-full">
              <h2 className="text-2xl font-bold text-black dark:text-white">
                Satılık Akaryakıt İstasyonu İlanları
              </h2>
            </div>
          </div>
