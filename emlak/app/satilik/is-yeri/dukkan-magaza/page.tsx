...
export const metadata: Metadata = {
  title: "Satılık Dükkan ve Mağaza İlanları | Tüzgen Group",
  description: "Satılık dükkan ve mağaza ilanlarını inceleyin, detaylı bilgi alın ve iletişime geçin.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?type=is-yeri&subtype=dukkan-magaza&status=satilik&limit=12`,
...
export default async function SatilikDukkanMagazaPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Dükkan ve Mağaza İlanları"
        description="Yatırım veya işletme amaçlı satılık dükkan ve mağaza ilanlarını inceleyin."
      />
