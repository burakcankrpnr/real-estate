...
export const metadata: Metadata = {
  title: "Satılık Çiftlik İlanları | Tüzgen Group",
  description: "Satılık çiftlik ilanlarını inceleyebilir, ticari yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=satilik&type=ciftlik&limit=24`,
...
export default async function SatilikCiftlikPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Çiftlik İlanları"
        description="Satılık çiftlik ilanlarını inceleyin ve ticari yatırım fırsatlarını değerlendirin."
      />
