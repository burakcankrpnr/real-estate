...
export const metadata: Metadata = {
  title: "Satılık Depo & Antrepo İlanları | Tüzgen Group",
  description: "Satılık depo ve antrepo ilanlarını inceleyebilir, ticari yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=satilik&type=depo-antrepo&limit=24`,
...
export default async function SatilikDepoAntrepoPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Depo & Antrepo İlanları"
        description="Satılık depo ve antrepo ilanlarını inceleyin ve ticari yatırım fırsatlarını değerlendirin."
      />
