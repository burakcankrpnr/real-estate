...
export const metadata: Metadata = {
  title: "Satılık Ofis İlanları | Tüzgen Group",
  description: "Satılık ofis ilanlarını inceleyin, bütçenize uygun ofisi bulun.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?type=is-yeri&subtype=ofis&status=satilik&limit=12`,
...
export default async function SatilikOfisPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Ofis İlanları"
        description="Satılık ofis ilanlarını inceleyin, bütçenize ve iş ihtiyaçlarınıza uygun ofisi bulun."
      />
