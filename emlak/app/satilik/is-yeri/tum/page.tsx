...
export const metadata: Metadata = {
  title: "Satılık İş Yeri İlanları | Tüzgen Group",
  description: "Satılık dükkan, ofis, mağaza ve diğer iş yeri ilanlarını keşfedin.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=satilik&limit=24`,
...
export default async function SatilikIsYeriPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık İş Yeri İlanları"
        description="Satılık dükkan, ofis, mağaza ve diğer iş yeri ilanlarını keşfedin."
      />
