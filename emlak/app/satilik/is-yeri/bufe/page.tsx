...
export const metadata: Metadata = {
  title: "Satılık Büfe İlanları | Tüzgen Group",
  description: "Satılık büfe ilanlarını inceleyebilir, ticari yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=satilik&type=bufe&limit=24`,
...
export default async function SatilikBufePage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Büfe İlanları"
        description="Satılık büfe ilanlarını inceleyin ve ticari yatırım fırsatlarını değerlendirin."
      />
