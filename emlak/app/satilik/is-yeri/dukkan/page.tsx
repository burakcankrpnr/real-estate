...
export const metadata: Metadata = {
  title: "Satılık Dükkan İlanları | Tüzgen Group",
  description: "Satılık dükkan ilanlarını inceleyin, bütçenize uygun dükkanı bulun.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?type=is-yeri&subtype=dukkan&status=satilik&limit=12`,
...
export default async function SatilikDukkanPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Dükkan İlanları"
        description="Satılık dükkan ilanlarını inceleyin, bütçenize ve iş ihtiyaçlarınıza uygun dükkanı bulun."
      />
