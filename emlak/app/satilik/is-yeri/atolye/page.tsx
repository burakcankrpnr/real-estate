...
export const metadata: Metadata = {
  title: "Satılık Atölye İlanları | Tüzgen Group",
  description: "Satılık atölye ilanlarını inceleyebilir, ticari yatırım fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=satilik&type=atolye&limit=24`,
...
export default async function SatilikAtolyePage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Satılık Atölye İlanları"
        description="Satılık atölye ilanlarını inceleyin ve ticari yatırım fırsatlarını değerlendirin."
      />
