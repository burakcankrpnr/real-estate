...
export const metadata: Metadata = {
  title: "Kiralık Zeytinlik İlanları | Tüzgen Group",
  description: "Kiralık zeytinlik ilanlarını inceleyebilir, uygun şartlarla zeytinlik kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=kiralik&type=zeytinlik&limit=24`,
...
export default async function KiralikZeytinlikPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Zeytinlik İlanları"
        description="Kiralık zeytinlik ilanlarını inceleyin ve zeytinlik kiralama fırsatlarını değerlendirin."
      />
