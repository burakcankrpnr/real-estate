...
export const metadata: Metadata = {
  title: "Kiralık Bahçe İlanları | Tüzgen Group",
  description: "Kiralık bahçe ilanlarını inceleyebilir, uygun şartlarla bahçe kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=kiralik&type=bahce&limit=24`,
...
export default async function KiralikBahcePage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Bahçe İlanları"
        description="Kiralık bahçe ilanlarını inceleyin ve bahçe kiralama fırsatlarını değerlendirin."
      />
