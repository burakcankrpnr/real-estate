...
export const metadata: Metadata = {
  title: "Kiralık Depo İlanları | Tüzgen Group",
  description: "Kiralık depo ilanlarını inceleyebilir, uygun şartlarla depo kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=kiralik&type=depo&limit=24`,
...
export default async function KiralikDepoPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Depo İlanları"
        description="Kiralık depo ilanlarını inceleyin ve depo kiralama fırsatlarını değerlendirin."
      />
