...
export const metadata: Metadata = {
  title: "Kiralık Bağ İlanları | Tüzgen Group",
  description: "Kiralık bağ ilanlarını inceleyebilir, uygun şartlarla bağ kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=kiralik&type=bag&limit=24`,
...
export default async function KiralikBagPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Bağ İlanları"
        description="Kiralık bağ ilanlarını inceleyin ve bağ kiralama fırsatlarını değerlendirin."
      />
