...
export const metadata: Metadata = {
  title: "Kiralık Arsa İlanları | Tüzgen Group",
  description: "Kiralık arsa ilanlarını inceleyebilir, uygun şartlarla arsa kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=kiralik&type=arsa&limit=24`,
...
export default async function KiralikArsaPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Arsa İlanları"
        description="Kiralık arsa ilanlarını inceleyin ve arsa kiralama fırsatlarını değerlendirin."
      />
