...
export const metadata: Metadata = {
  title: "Kiralık Tarla İlanları | Tüzgen Group",
  description: "Kiralık tarla ilanlarını inceleyebilir, uygun şartlarla tarla kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=arsa&status=kiralik&type=tarla&limit=24`,
...
export default async function KiralikTarlaPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Tarla İlanları"
        description="Kiralık tarla ilanlarını inceleyin ve tarla kiralama fırsatlarını değerlendirin."
      />
