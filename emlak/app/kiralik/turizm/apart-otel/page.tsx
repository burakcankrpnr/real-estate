...
export const metadata: Metadata = {
  title: "Kiralık Apart Otel İlanları | Tüzgen Group",
  description: "Kiralık apart otel ilanlarını inceleyebilir, uygun şartlarla apart otel kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=kiralik&type=apart-otel&limit=24`,
...
export default async function KiralikApartOtelPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Apart Otel İlanları"
        description="Kiralık apart otel ilanlarını inceleyin ve apart otel kiralama fırsatlarını değerlendirin."
      />
