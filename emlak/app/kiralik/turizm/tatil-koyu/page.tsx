...
export const metadata: Metadata = {
  title: "Kiralık Tatil Köyü İlanları | Tüzgen Group",
  description: "Kiralık tatil köyü ilanlarını inceleyebilir, uygun şartlarla tatil köyü kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=kiralik&type=tatil-koyu&limit=24`,
...
export default async function KiralikTatilKoyuPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Tatil Köyü İlanları"
        description="Kiralık tatil köyü ilanlarını inceleyin ve tatil köyü kiralama fırsatlarını değerlendirin."
      />
