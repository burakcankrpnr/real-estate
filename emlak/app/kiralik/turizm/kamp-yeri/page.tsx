...
export const metadata: Metadata = {
  title: "Kiralık Kamp Yeri İlanları | Tüzgen Group",
  description: "Kiralık kamp yeri ilanlarını inceleyebilir, uygun şartlarla kamp yeri kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=kiralik&type=kamp-yeri&limit=24`,
...
export default async function KiralikKampYeriPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Kamp Yeri İlanları"
        description="Kiralık kamp yeri ilanlarını inceleyin ve kamp yeri kiralama fırsatlarını değerlendirin."
      />
