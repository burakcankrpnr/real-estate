...
export const metadata: Metadata = {
  title: "Kiralık Motel İlanları | Tüzgen Group",
  description: "Kiralık motel ilanlarını inceleyebilir, uygun şartlarla motel kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=kiralik&type=motel&limit=24`,
...
export default async function KiralikMotelPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Motel İlanları"
        description="Kiralık motel ilanlarını inceleyin ve motel kiralama fırsatlarını değerlendirin."
      />
