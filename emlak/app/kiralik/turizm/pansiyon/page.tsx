...
export const metadata: Metadata = {
  title: "Kiralık Pansiyon İlanları | Tüzgen Group",
  description: "Kiralık pansiyon ilanlarını inceleyebilir, uygun şartlarla pansiyon kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=kiralik&type=pansiyon&limit=24`,
...
export default async function KiralikPansiyonPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Pansiyon İlanları"
        description="Kiralık pansiyon ilanlarını inceleyin ve pansiyon kiralama fırsatlarını değerlendirin."
      />
