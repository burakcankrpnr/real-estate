...
export const metadata: Metadata = {
  title: "Kiralık Butik Otel İlanları | Tüzgen Group",
  description: "Kiralık butik otel ilanlarını inceleyebilir, uygun şartlarla butik otel kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=kiralik&type=butik-otel&limit=24`,
...
export default async function KiralikButikOtelPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Butik Otel İlanları"
        description="Kiralık butik otel ilanlarını inceleyin ve butik otel kiralama fırsatlarını değerlendirin."
      />
