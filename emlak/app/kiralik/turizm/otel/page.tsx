...
export const metadata: Metadata = {
  title: "Kiralık Otel İlanları | Tüzgen Group",
  description: "Kiralık otel ilanlarını inceleyebilir, turizm sektöründeki fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=kiralik&type=otel&limit=24`,
...
export default async function KiralikOtelPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Otel İlanları"
        description="Kiralık otel ilanlarını inceleyin ve turizm sektöründeki fırsatları değerlendirin."
      />
