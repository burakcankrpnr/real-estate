...
export const metadata: Metadata = {
  title: "Kiralık Turizm Tesisleri | Tüzgen Group",
  description: "Kiralık otel, pansiyon, tatil köyü ve diğer turizm tesisi ilanlarını keşfedin.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=turizm&status=kiralik&limit=24`,
...
export default async function KiralikTurizmPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Turizm Tesisleri"
        description="Kiralık otel, pansiyon, tatil köyü ve diğer turizm tesisi ilanlarını keşfedin."
      />
