...
export const metadata: Metadata = {
  title: "Kiralık Dükkan/Mağaza İlanları | Tüzgen Group",
  description: "Kiralık dükkan ve mağaza ilanlarını inceleyebilir, ticari kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=kiralik&type=dukkan-magaza&limit=24`,
...
export default async function KiralikDukkanMagazaPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Dükkan/Mağaza İlanları"
        description="Kiralık dükkan ve mağaza ilanlarını inceleyin ve ticari kiralama fırsatlarını değerlendirin."
      />
          ...
          <div className="flex flex-wrap justify-between">
            <div className="mb-8 w-full">
              <h2 className="text-2xl font-bold text-black dark:text-white">
                Kiralık Dükkan/Mağaza İlanları
              </h2>
            </div>
          </div>
