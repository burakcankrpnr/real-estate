...
export const metadata: Metadata = {
  title: "Kiralık Çiftlik İlanları | Tüzgen Group",
  description: "Kiralık çiftlik ilanlarını inceleyebilir, tarım ve hayvancılık işletme kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=kiralik&type=ciftlik&limit=24`,
...
export default async function KiralikCiftlikIsYeriPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Çiftlik İlanları"
        description="Kiralık çiftlik ilanlarını inceleyin ve tarım işletmesi kiralama fırsatlarını değerlendirin."
      />
