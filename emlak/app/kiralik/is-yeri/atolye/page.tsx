...
export const metadata: Metadata = {
  title: "Kiralık Atölye İlanları | Tüzgen Group",
  description: "Kiralık atölye ilanlarını inceleyebilir, üretim ve imalat alanı kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=kiralik&type=atolye&limit=24`,
...
export default async function KiralikAtolyePage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Atölye İlanları"
        description="Kiralık atölye ilanlarını inceleyin ve üretim alanı kiralama fırsatlarını değerlendirin."
      />
