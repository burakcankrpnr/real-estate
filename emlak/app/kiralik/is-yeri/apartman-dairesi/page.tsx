...
export const metadata: Metadata = {
  title: "Kiralık Apartman Dairesi İş Yeri İlanları | Tüzgen Group",
  description: "Kiralık apartman dairesi iş yeri ilanlarını inceleyebilir, ticari gayrimenkul kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=kiralik&type=apartman-dairesi&limit=24`,
...
export default async function KiralikApartmanDairesiIsYeriPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Apartman Dairesi İş Yeri İlanları"
        description="Kiralık apartman dairesi iş yeri ilanlarını inceleyin ve ticari gayrimenkul kiralama fırsatlarını değerlendirin."
      />
          ...
          <div className="flex flex-wrap justify-between">
            <div className="mb-8 w-full">
              <h2 className="text-2xl font-bold text-black dark:text-white">
                Kiralık Apartman Dairesi İş Yeri İlanları
              </h2>
            </div>
          </div>
