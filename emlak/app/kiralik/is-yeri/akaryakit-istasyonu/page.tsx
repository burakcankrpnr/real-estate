...
export const metadata: Metadata = {
  title: "Kiralık Akaryakıt İstasyonu İlanları | Tüzgen Group",
  description: "Kiralık akaryakıt istasyonu ilanlarını inceleyebilir, ticari işletme kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=kiralik&type=akaryakit-istasyonu&limit=24`,
...
export default async function KiralikAkaryakitIstasyonuPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Akaryakıt İstasyonu İlanları"
        description="Kiralık akaryakıt istasyonu ilanlarını inceleyin ve ticari yatırım fırsatlarını değerlendirin."
      />
          ...
          <div className="flex flex-wrap justify-between">
            <div className="mb-8 w-full">
              <h2 className="text-2xl font-bold text-black dark:text-white">
                Kiralık Akaryakıt İstasyonu İlanları
              </h2>
            </div>
          </div>
