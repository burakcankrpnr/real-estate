...
export const metadata: Metadata = {
  title: "Kiralık İş Yeri İlanları | Tüzgen Group",
  description: "Kiralık dükkan, ofis, mağaza ve diğer iş yeri ilanlarını keşfedin.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=kiralik&limit=24`,
...
export default async function KiralikIsYeriPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık İş Yeri İlanları"
        description="Kiralık dükkan, ofis, mağaza ve diğer iş yeri ilanlarını keşfedin."
      />
