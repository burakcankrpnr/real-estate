...
export const metadata: Metadata = {
  title: "Kiralık Depo & Antrepo İlanları | Tüzgen Group",
  description: "Kiralık depo ve antrepo ilanlarını inceleyebilir, ticari depolama alanı kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=kiralik&type=depo-antrepo&limit=24`,
...
export default async function KiralikDepoAntrepoPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Depo & Antrepo İlanları"
        description="Kiralık depo ve antrepo ilanlarını inceleyin ve ticari depolama alanı kiralama fırsatlarını değerlendirin."
      />
