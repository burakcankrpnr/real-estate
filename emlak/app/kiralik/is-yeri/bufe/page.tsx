...
export const metadata: Metadata = {
  title: "Kiralık Büfe İlanları | Tüzgen Group",
  description: "Kiralık büfe ilanlarını inceleyebilir, küçük işletme kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=is-yeri&status=kiralik&type=bufe&limit=24`,
...
export default async function KiralikBufePage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Büfe İlanları"
        description="Kiralık büfe ilanlarını inceleyin ve küçük işletme kiralama fırsatlarını değerlendirin."
      />
