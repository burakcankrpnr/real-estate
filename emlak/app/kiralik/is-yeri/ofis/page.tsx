...
export const metadata: Metadata = {
  title: "Kiralık Ofis İlanları | Tüzgen Group",
  description: "Kiralık ofis ilanlarını inceleyin, işletmeniz için uygun ofis bulun.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?type=is-yeri&subtype=ofis&status=kiralik&limit=12`,
...
export default async function KiralikOfisPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Ofis İlanları"
        description="Kiralık ofis ilanlarını inceleyin, bütçenize ve iş ihtiyaçlarınıza uygun ofisi bulun."
      />
