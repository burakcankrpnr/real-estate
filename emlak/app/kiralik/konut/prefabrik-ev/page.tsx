...
export const metadata: Metadata = {
  title: "Kiralık Prefabrik Ev İlanları | Tüzgen Group",
  description: "Kiralık prefabrik ev ilanlarını inceleyebilir, ekonomik ve hızlı konut kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=kiralik&type=prefabrik-ev&limit=24`,
...
export default async function KiralikPrefabrikEvPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Prefabrik Ev İlanları"
        description="Kiralık prefabrik ev ilanlarını inceleyin ve ekonomik konut kiralama fırsatlarını değerlendirin."
      />
