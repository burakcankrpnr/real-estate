...
export const metadata: Metadata = {
  title: "Kiralık Villa İlanları | Tüzgen Group",
  description: "Kiralık villa ilanlarını inceleyebilir, lüks konut kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=kiralik&type=villa&limit=24`,
...
export default async function KiralikVillaPage() {
  ...
  return (
    <>
      <Breadcrumb
        pageName="Kiralık Villa İlanları"
        description="Kiralık villa ilanlarını inceleyin ve lüks konut kiralama fırsatlarını değerlendirin."
      />
