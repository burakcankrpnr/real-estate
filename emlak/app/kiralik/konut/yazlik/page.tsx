...
export const metadata: Metadata = {
  title: "Kiralık Yazlık İlanları | Tüzgen Group",
  description: "Kiralık yazlık ilanlarını inceleyebilir, sezonluk konut kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=kiralik&type=yazlik&limit=24`,
...
export default async function KiralikYazlikPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Yazlık İlanları"
        description="Kiralık yazlık ilanlarını inceleyin ve sezonluk konut kiralama fırsatlarını değerlendirin."
      />
