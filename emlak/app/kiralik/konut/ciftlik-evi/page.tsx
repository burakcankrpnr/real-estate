...
export const metadata: Metadata = {
  title: "Kiralık Çiftlik Evi İlanları | Tüzgen Group",
  description: "Kiralık çiftlik evi ilanlarını inceleyebilir, kırsal konut kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=kiralik&type=ciftlik-evi&limit=24`,
...
export default async function KiralikCiftlikEviPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Çiftlik Evi İlanları"
        description="Kiralık çiftlik evi ilanlarını inceleyin ve kırsal konut kiralama fırsatlarını değerlendirin."
      />
