...
export const metadata: Metadata = {
  title: "Kiralık Rezidans İlanları | Tüzgen Group",
  description: "Kiralık rezidans ilanlarını inceleyebilir, lüks konut kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=kiralik&type=rezidans&limit=24`,
...
export default async function KiralikRezidansPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Rezidans İlanları"
        description="Kiralık rezidans ilanlarını inceleyin ve lüks konut kiralama fırsatlarını değerlendirin."
      />
