...
export const metadata: Metadata = {
  title: "Kiralık Müstakil Ev İlanları | Tüzgen Group",
  description: "Kiralık müstakil ev ilanlarını inceleyebilir, bağımsız konut kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=kiralik&type=mustakil-ev&limit=24`,
...
export default async function KiralikMustakilEvPage() {
  const properties: Property[] = await getProperties();

  return (
    <>
      <Breadcrumb
        pageName="Kiralık Müstakil Ev İlanları"
        description="Kiralık müstakil ev ilanlarını inceleyin ve bağımsız konut kiralama fırsatlarını değerlendirin."
      />
