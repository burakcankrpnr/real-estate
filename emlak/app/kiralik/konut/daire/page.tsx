...
export const metadata: Metadata = {
  title: "Kiralık Daire İlanları | Tüzgen Group",
  description: "Kiralık daire ilanlarını inceleyebilir, konut kiralama fırsatlarını değerlendirebilirsiniz.",
};

async function getProperties() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/properties?category=konut&status=kiralik&type=daire&limit=24`,
...
export default async function KiralikDairePage() {
  ...
  return (
    <>
      <Breadcrumb
        pageName="Kiralık Daire İlanları"
        description="Kiralık daire ilanlarını inceleyin ve konut kiralama fırsatlarını değerlendirin."
      />
